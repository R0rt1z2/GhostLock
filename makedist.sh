...
if [ -f "$here/flash_preloader.tar" ]; then
  cp "$here/downgrade.sh" "$stage/"
  cp "$here/downgrade.ps1" "$stage/"
  cp "$here/downgrade.bat" "$stage/"
  cp "$here/flash_preloader.tar" "$stage/"
  chmod 755 "$stage/downgrade.sh"
else
  echo "[!] no flash_preloader.tar; packaging without the downgrade tooling"
fi
